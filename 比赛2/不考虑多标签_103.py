...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.cc = nn.Linear(768, 105)
        self.dropout = nn.Dropout(0.5)  # 添加dropout层
        self.crf = CRF(105)  # 添加CRF层
...
def train_model(learning_rate, num_epochs):
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for i, (input_ids, attention_mask, labels) in enumerate(train_loader):
                ...
                for j in range(len(labels)):
                    ...
                    t_first_index = true_label.index(104)
                    t_second_index = true_label.index(104, t_first_index + 1)
            ...
            if val_f1 > best_val_f1:
                ...
                early_path = "early_3.pt"
                torch.save(model.state_dict(), early_path)
                print(f"最佳保存至：{early_path}")
                counter = 0
            else:
                counter += 1
        ...
        print('手动终止训练')
        model_save_path = "c_model_interrupted.pth"
