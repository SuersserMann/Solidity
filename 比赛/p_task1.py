...
def collate_fn(data):
    ...
    return input_ids, attention_mask, labels, sentence_ids
...
sentence_id_1=[]
with torch.no_grad():
    for i, (input_ids, attention_mask, labels, sentence_id) in enumerate(test_loader):
        out = model(input_ids=input_ids, attention_mask=attention_mask)
        loss = criterion(out, labels)  # 计算损失
        out = out.argmax(dim=1)
        predicted_labels = []
        true_labels = []
        for z in range(len(sentence_id)):
            sentence_id_1.append(sentence_id[z])
