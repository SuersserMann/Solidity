    ...
    for i in data:
        ...
        for yy, item in enumerate(cfn_spanss_one):
            if targets_one['start'] < item['start']:
                xl_text = texts_one[item['start']:item['end'] + 1]
                string_list = [str(num) for num in xl_text]
                string_list = char_list + ['空'] * (item['start'] - targets_one['end'] - 1) + string_list
                result_list.append(string_list)
            else:
                xl_text = texts_one[item['start']:item['end'] + 1]
                string_list = [str(num) for num in xl_text]
                string_list = string_list + ['空'] * (targets_one['start'] - item['end'] - 1) + char_list
                result_list.append(string_list)
    ...
    try:
        for epoch in range(num_epochs):
            ...
            if counter >= patience:
                ...
                # 保存当前模型
                torch.save(best_model_state, "task3_model_early_3.pt")
                break
            lr_scheduler.step()
...
# 保存训练好的模型
model_save_path = "task3_best_model_3.pth"
