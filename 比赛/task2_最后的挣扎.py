...
class Model(torch.nn.Module):
    def __init__(self):
        ...
        self.dropout = nn.Dropout(0.2)  # 添加Dropout层，丢弃概率为0.2
...
def collate_fn(data):
    ...
    for i in data:
        ...
        for u in range(len(words_one)):
            if words_one[u]['pos'] == 'r':
                for x in range(words_one[u]['start'], words_one[u]['end'] + 1):
                    string_list[x] = texts_one[x]
        for u in range(len(words_one)):
            if words_one[u]['pos'] == 'u':
                for x in range(words_one[u]['start'], words_one[u]['end'] + 1):
                    string_list[x] = texts_one[x]
        for u in range(len(words_one)):
            if words_one[u]['pos'] == 'p':
                for x in range(words_one[u]['start'], words_one[u]['end'] + 1):
                    string_list[x] = texts_one[x]
    ...
    try:
        for epoch in range(num_epochs):
            ...
            if counter >= patience:
                ...
                # 保存当前模型
                torch.save(best_model_state, "task2_model_early_13.pt")
                break
            lr_scheduler.step()
...
# 保存训练好的模型
model_save_path = "task2_best_model_33.pth"
